...
The standalone site publishes `ads.txt` at its root with the matching Google publisher ID. After deployment, verify `https://dyut.siddheshpadhye.co.in/ads.txt` returns `200 OK` and use AdSense's **Check for updates** action if the status remains pending.
