...
const MoveSelector = ({ roll, validMoves, onFullMove, onSplitMove, onClose, onInitiatePairAttack, onDualSpawnAttack, possiblePairAttacks, onNextRoll, hasMultipleRolls, title }) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
        <div className="glass-panel border-[1.5px] border-white/20 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] p-6 flex flex-col gap-3 min-w-[260px] animate-hop" onClick={(e) => e.stopPropagation()}>
          ...
          {validMoves.dualSpawn ? (
            <button
              onClick={onDualSpawnAttack}
              className="w-full px-4 py-3 bg-ruby/90 text-white font-sans font-bold rounded-xl hover:bg-ruby shadow-[0_0_15px_rgba(220,38,38,0.5)] border border-ruby/50 animate-pulse transition-all"
            >
              {t('dualSpawnAttack', 'Dual Spawn Attack!')}
            </button>
          ) : (
            <button
